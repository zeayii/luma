...
public sealed class LumaEngine<TState>
{
        ...
        BranchPolicy branchPolicy)
    {
        ...
        var runtime = new LumaNodeRuntime<TState>(
            ...
            executionProfile,
            parentToken);
        var inheritedBranchRootRuntime = parentRuntime?.BranchRootRuntime;
        var branchRootRuntime = branchPolicy == BranchPolicy.NewBranch || inheritedBranchRootRuntime is null ? runtime : inheritedBranchRootRuntime;
        runtime.BindBranchRoot(branchRootRuntime);
        ...
        {
            await HandleStopExceptionAsync(stopException, runtime, "Node build-request phase stopped by business rule.").ConfigureAwait(false);
        }
    ...
    private async Task RequestWorkerAsync(NodeTaskScheduler requestScheduler, NodeTaskScheduler downloadScheduler, PriorityTaskScheduler<ItemEnvelope<TState>> persistScheduler, LumaRunRuntime runRuntime)
    {
        try
        {
            while (!runRuntime.Token.IsCancellationRequested)
            {
                ...
                {
                    await HandleStopExceptionAsync(stopException, runtime, "Node response phase stopped by business rule.").ConfigureAwait(false);
                }
                catch (OperationCanceledException) when (runtime.CancellationTokenSource.IsCancellationRequested || runRuntime.Token.IsCancellationRequested)
                {
                }
    ...
    private async Task DownloadWorkerAsync(NodeTaskScheduler downloadScheduler, NodeTaskScheduler requestScheduler, PriorityTaskScheduler<ItemEnvelope<TState>> persistScheduler, LumaRunRuntime runRuntime)
    {
        try
        {
            while (!runRuntime.Token.IsCancellationRequested)
            {
                ...
                {
                    await HandleStopExceptionAsync(stopException, runtime, "Node download phase stopped by business rule.").ConfigureAwait(false);
                }
                catch (OperationCanceledException) when (runtime.CancellationTokenSource.IsCancellationRequested || runRuntime.Token.IsCancellationRequested)
                {
                    // ignore
        ...
        CancellationToken cancellationToken)
    {
        ...
        for (var index = 0; index < buffer.Count; index++)
        {
            ...
            {
                await HandleStopExceptionAsync(stopException, runtime, "Node persist filter phase stopped by business rule.").ConfigureAwait(false);
        ...
        for (var index = 0; index < buffer.Count; index++)
        {
            ...
            {
                await HandleStopExceptionAsync(stopException, runtime, "Node persisted callback phase stopped by business rule.").ConfigureAwait(false);
            }
            catch (Exception exception)
            {
            ...
            runtime.CancelBranch(persistResult.Message);
            SignalStateChanged(runtime);
        }
    ...
    {
        const int completionQuietWindowMilliseconds = 100;
        while (!runRuntime.Token.IsCancellationRequested)
        {
            var stateVersionBeforeCheck = Volatile.Read(ref _stateVersion);
            if (IsRunCompleted(requestScheduler, downloadScheduler))
            {
                await Task.Delay(completionQuietWindowMilliseconds, runRuntime.Token).ConfigureAwait(false);

                var stateVersionAfterQuietWindow = Volatile.Read(ref _stateVersion);
                if (stateVersionAfterQuietWindow != stateVersionBeforeCheck || !IsRunCompleted(requestScheduler, downloadScheduler))
                {
                    continue;
                }
    ...
    private bool IsRunCompleted(NodeTaskScheduler requestScheduler, NodeTaskScheduler downloadScheduler)
    {
        ...
        if (!_dirtyRuntimePaths.IsEmpty || !_dirtyRuntimePathSet.IsEmpty)
        {
            return false;
        }

        return _nodeRuntimes.Values.All(static runtime => runtime is
        {
            State: { ActiveRequestCount: <= 0, QueuedRequestCount: <= 0, Status: not NodeExecutionStatus.Pending },
    ...
    private void TryCompleteNodeSubtrees()
    {
        while (_dirtyRuntimePaths.TryDequeue(out var path))
        {
            ...
            if (!runtime.TryBeginSubtreeCompletion())
            {
                continue;
            }
    ...
    /// <returns>异步任务。</returns>
    private ValueTask HandleStopExceptionAsync(LumaStopException exception, LumaNodeRuntime<TState> runtime, string phase)
    {
        var reason = $"[{exception.Code}] {exception.Message}";
        runtime.Cancel(reason);
        SignalStateChanged(runtime);
        WriteUnifiedLog(LogLevelKind.Warning, "Engine", $"Event=StopExceptionNodeHandled Phase={phase} NodePath={runtime.Path} Reason={reason}");
