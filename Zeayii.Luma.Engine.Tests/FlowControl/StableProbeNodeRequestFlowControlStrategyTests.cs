...
public sealed class StableProbeNodeRequestFlowControlStrategyTests
{
    ...
    public void ObserveResponseShouldBackoffFastAndRecoverSlowlyAfterCooldown()
    {
        ...
        strategy.Update(new NodeRequestFlowControlStrategyOptions(
            ...
            AdaptiveBackoffStatusCodes: [429],
            AdaptiveBackoffMaxHits: 0,
            AdaptiveMaxIntervalMilliseconds: 0));

        Assert.Equal(10, strategy.ResolveEffectiveMinIntervalMilliseconds());

        strategy.ObserveResponse(HttpStatusCode.TooManyRequests, nowUtcMilliseconds: 0);
        Assert.Equal(20, strategy.ResolveEffectiveMinIntervalMilliseconds());

        strategy.ObserveResponse(HttpStatusCode.OK, nowUtcMilliseconds: 1_000);
        Assert.Equal(20, strategy.ResolveEffectiveMinIntervalMilliseconds());

        for (var index = 0; index < 48; index++)
        {
            strategy.ObserveResponse(HttpStatusCode.OK, nowUtcMilliseconds: 30_000 + index);
        }
    ...
    public void ObserveResponseShouldStopEscalationWhenMaxHitsReached()
    {
        ...
        strategy.Update(new NodeRequestFlowControlStrategyOptions(
            ...
            AdaptiveBackoffEnabled: true,
            AdaptiveBackoffStatusCodes: [429],
            AdaptiveBackoffMaxHits: 2,
            AdaptiveMaxIntervalMilliseconds: 0));

        strategy.ObserveResponse(HttpStatusCode.TooManyRequests, nowUtcMilliseconds: 0);
        Assert.Equal(20, strategy.ResolveEffectiveMinIntervalMilliseconds());

        strategy.ObserveResponse(HttpStatusCode.TooManyRequests, nowUtcMilliseconds: 1);
        Assert.Equal(40, strategy.ResolveEffectiveMinIntervalMilliseconds());

        strategy.ObserveResponse(HttpStatusCode.TooManyRequests, nowUtcMilliseconds: 2);
        Assert.Equal(40, strategy.ResolveEffectiveMinIntervalMilliseconds());
    }
